...
export async function listCategories(
  params: ListParams,
): Promise<Paginated<{ id: number; publicId: string; title: string }>> {
  ...
  const where = params.q?.trim()
    ? like(categoryTable.title, `%${params.q.trim()}%`)
    : undefined;
...
export async function listTenants(
  params: ListParams,
): Promise<Paginated<{ id: number; publicId: string; name: string }>> {
  const { user } = await validateRequest();
  if (!user) throw new Error("Unauthorized");
  requireEditorOrAdmin(user.role as any);
  const page = Math.max(1, params.page || 1);
  ...
  const where = params.q?.trim()
    ? like(tenantTable.name, `%${params.q.trim()}%`)
    : undefined;
...
export async function listCollections(
  ...
  Paginated<{ id: number; publicId: string; tenantId: number; title: string }>
> {
  const { user } = await validateRequest();
  if (!user) throw new Error("Unauthorized");
  requireEditorOrAdmin(user.role as any);
  ...
  const where = params.q?.trim()
    ? like(collectionTable.title, `%${params.q.trim()}%`)
    : undefined;
  ...
  const [rows, total] = await Promise.all([
    await (where
      ? db
          ...
          .where(where)
          .orderBy(asc(collectionTable.title))
          .limit(pageSize)
          .offset(offset)
      : db
          .select()
...
export async function updateCollection(input: { id: number; title: string }) {
  ...
  await db
    ...
    .set({ title: input.title })
    .where(eq(collectionTable.id, input.id));
}
