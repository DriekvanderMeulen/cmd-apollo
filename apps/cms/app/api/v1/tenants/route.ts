...
export async function GET(req: NextRequest) {
    ...
    const [rows, total] = await Promise.all([
        (where
            ? db.select().from(tenantTable).where(where).orderBy(desc(tenantTable.name)).limit(pageSize).offset(offset)
            : db.select().from(tenantTable).orderBy(desc(tenantTable.name)).limit(pageSize).offset(offset)),
        db.select({ value: count() }).from(tenantTable).where(where as any),
    ])
