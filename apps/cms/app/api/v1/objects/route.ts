...
export async function GET(req: NextRequest) {
  ...
  const where = and(
    eq(objectTable.userId, user.id),
    q ? like(objectTable.title, `%${q}%`) : undefined,
  );
  ...
  const [rows, total] = await Promise.all([
    db
      ...
      .leftJoin(
        collectionTable,
        eq(objectTable.collectionId, collectionTable.id),
      )
...
export async function POST(req: NextRequest) {
  ...
  const existing = await db
    ...
    .where(
      and(
        eq(objectTable.userId, user.id),
        eq(objectTable.collectionId, Number(body.collectionId)),
      ),
    );
  if (existing.length > 0) {
    return NextResponse.json(
      { error: "Object for this collection already exists" },
      { status: 409 },
    );
  }
  ...
  const insert = await db.insert(objectTable).values({
    ...
    // Note: videoR2Key will be set later when video is uploaded
    // Temporarily omitting videoR2Key until database schema is updated
  });
