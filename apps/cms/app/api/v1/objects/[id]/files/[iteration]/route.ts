...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string; iteration: string }> }) {
  const { user } = await validateRequest()
  if (!user) return NextResponse.json({ error: 'Unauthorized' }, { status: 401 })
  const { id: idRaw, iteration: itRaw } = await params
...
export async function DELETE(_req: NextRequest, { params }: { params: Promise<{ id: string; iteration: string }> }) {
  const { user } = await validateRequest()
  if (!user) return NextResponse.json({ error: 'Unauthorized' }, { status: 401 })
  const { id: idRaw, iteration: itRaw } = await params
  const id = Number(idRaw)
  ...
  const client = createR2Client()
  await client.send(new DeleteObjectCommand({ Bucket: R2_BUCKET, Key: buildKey(obj.cfR2Link || null, obj.collectionId, obj.userId, obj.id, iteration) }))
