...
export async function GET(req: Request): Promise<Response> {
    ...
    if (categoryIdParam) wheres.push(eq(objectTable.categoryId, Number(categoryIdParam)))
    if (collectionIdParam) wheres.push(eq(objectTable.collectionId, Number(collectionIdParam)))
    ...
    const data = page.map((r) => ({
        ...
        cfR2Link: r.cfR2Link,
        tenantId: r.tenantId,
        username: r.userGivenName || r.userFamilyName ? `${r.userGivenName ?? ''} ${r.userFamilyName ?? ''}`.trim() : r.userEmail,
