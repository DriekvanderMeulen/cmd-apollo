...
export async function GET(req: Request, { params }: { params: Promise<{ publicId: string }> }): Promise<Response> {
	...
	const rows = await db
		...
		.from(objectTable)
		.where(and(eq(objectTable.publicId, publicId), eq(objectTable.public, true)))
