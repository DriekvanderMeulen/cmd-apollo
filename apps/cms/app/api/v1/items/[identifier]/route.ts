...
type IdentifierParams = Promise<{ identifier: string }>;
...
export async function GET(
  ...
  const item = await loadItem({ type: "id", value: identifier });
  if (!item) {
...
async function loadItem(identifier: { type: "id" | "slug"; value: string }) {
  const whereClause =
    identifier.type === "id"
      ? and(eq(objectTable.publicId, identifier.value), eq(objectTable.public, true))
      : and(eq(objectTable.publicId, identifier.value), eq(objectTable.public, true));
...
function extractPlainText(value: any): string {
  ...
  if (typeof value === "object") {
    ...
    if (Array.isArray(value.content)) {
      return value.content.map(extractPlainText).join(" ").trim();
    }
  }
  return "";
}
