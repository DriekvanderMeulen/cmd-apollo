...
import { HiMiniMagnifyingGlass, HiChevronLeft, HiChevronRight, HiMiniPlus } from "react-icons/hi2"
...
function listCategories(params: { page: number; pageSize: number; q: string }) {
    const qs = new URLSearchParams({ page: String(params.page), pageSize: String(params.pageSize), q: params.q })
...
function updateCategory(input: { id: number; title: string }) {
    return apiJson(`/api/v1/categories/${input.id}`, 'PATCH', { title: input.title })
}
...
function listCollections(params: { page: number; pageSize: number; q: string }) {
    const qs = new URLSearchParams({ page: String(params.page), pageSize: String(params.pageSize), q: params.q })
...
function updateCollection(input: { id: number; title: string }) {
    return apiJson(`/api/v1/collections/${input.id}`, 'PATCH', { title: input.title })
}
...
function Pager({ page, total, onPage }: { page: number; total: number; onPage: (p: number) => void }) {
	...
	return (
		<div className="flex items-center space-x-2">
			<button className="p-1 rounded-[8px] hover:bg-neutral-100 disabled:opacity-50" onClick={() => onPage(Math.max(1, page - 1))} disabled={page <= 1}>Prev</button>
			<span className="text-sm text-neutral-600">Page {page} of {totalPages}</span>
			<button className="p-1 rounded-[8px] hover:bg-neutral-100 disabled:opacity-50" onClick={() => onPage(Math.min(totalPages, page + 1))} disabled={page >= totalPages}>Next</button>
		</div>
	)
}
...
function SectionHeader({ title, onCreate }: { title: string; onCreate: () => void }) {
	return (
		<div className="flex items-center justify-between">
			<button onClick={onCreate} className="inline-flex items-center gap-2 rounded-[8px] bg-neutral-900 px-3.5 py-2 text-black">
				<HiMiniPlus />
				Create
			</button>
		</div>
...
export function EditorPanel() {
	...
	useEffect(() => {
		...
		;(async () => {
			try {
				if (tab === "categories") {
					const res = await listCategories({ page, pageSize: PAGE_SIZE, q: query })
					if (cancelled) return
					setItems(res.items)
					setTotal(res.total)
				} else if (tab === "tenants") {
					const res = await listTenants({ page, pageSize: PAGE_SIZE, q: query })
					if (cancelled) return
					// Match tenant selector sorting by extracting year CMD-[A-C]-YYYY
					const extractYear = (name: string): number => {
						const match = name.match(/CMD-[A-C]-(\d{4})/)
						return match ? parseInt(match[1], 10) : 0
					}
					const sorted = [...res.items].sort((a, b) => extractYear((b.name || b.title) ?? '') - extractYear((a.name || a.title) ?? ''))
					setItems(sorted)
					setTotal(res.total)
				} else {
					const res = await listCollections({ page, pageSize: PAGE_SIZE, q: query })
					if (cancelled) return
					setItems(res.items)
					setTotal(res.total)
				}
			...
			(async () => {
				try {
					if (dialogMode === "create") {
						if (tab === "categories") await createCategory({ title: titleTrimmed })
						else if (tab === "tenants") await createTenant({ name: titleTrimmed })
						else {
							const tenantId = Number(inputTenantId)
							if (!Number.isFinite(tenantId)) return toast.error("Valid tenant ID required")
							await createCollection({ title: titleTrimmed, tenantId })
						}
						toast.success("Created")
						setParam("page", "1")
					} else if (activeItem) {
						if (tab === "categories") await updateCategory({ id: activeItem.id, title: titleTrimmed })
						else if (tab === "tenants") await updateTenant({ id: activeItem.id, name: titleTrimmed })
						else await updateCollection({ id: activeItem.id, title: titleTrimmed })
	...
	return (
		...
		<div className="space-y-6">
			<div>
				...
				<p className="text-neutral-600 mt-1">Manage categories, tenants and collections.</p>
			</div>
			...
			<div className="space-y-4">
				...
				<SectionHeader title={tab.charAt(0).toUpperCase() + tab.slice(1)} onCreate={openCreateDialog} />
				</div>
				...
				<div className="overflow-hidden rounded-[8px] border border-neutral-200 bg-white">
					<table className="min-w-full">
						<thead className="bg-neutral-50">
							<tr>
								<th className="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-neutral-500">Select</th>
						...
						<tbody>
							{items.length === 0 ? (
								<tr>
									<td colSpan={3} className="px-6 py-12 text-center text-neutral-500">No results.</td>
								</tr>
							) : (
								items.map((item) => (
									<tr key={item.id} className="border-b border-neutral-100 hover:bg-neutral-50">
										<td className="px-6 py-4">
											<input type="checkbox" aria-label={`Select ${item.title || item.name}`} checked={selectedIds.includes(item.id)} onChange={() => toggleSelectOne(item.id)} />
										</td>
										<td className="px-6 py-4 text-neutral-900">{item.title || item.name}</td>
										<td className="px-6 py-4">
											<div className="flex items-center gap-2">
												<button className="px-2 py-1 rounded-[8px] hover:bg-neutral-100" onClick={() => openEditDialog(item)} disabled={isPending}>{isPending ? <Spinner size={14} /> : "Edit"}</button>
												<button className="px-2 py-1 rounded-[8px] hover:bg-neutral-100 text-red-700" onClick={() => handleDelete(item)} disabled={isPending}>Delete</button>
											</div>
										</td>
									</tr>
								))
				...
				<div className="flex items-center justify-between text-sm text-neutral-600">
					...
					<div className="flex items-center gap-2">
						{selectedIds.length > 0 ? (
							<Button title={`Delete ${selectedIds.length} selected`} variant="danger" onClick={handleBulkDelete} />
						) : null}
		...
		{/* Create/Edit Dialog */}
		<Dialog title={dialogMode === "create" ? `Create ${tab.slice(0,1).toUpperCase()+tab.slice(1)}` : `Edit ${tab.slice(0,1).toUpperCase()+tab.slice(1)}`}
			isOpen={isDialogOpen}
			setIsOpen={setIsDialogOpen}
		>
			<div className="space-y-4">
				<label className="block">
					<div className="mb-1 text-sm font-medium text-neutral-700">{tab === "tenants" ? "Name" : "Title"}</div>
					<input
						type="text"
						value={inputTitle}
						onChange={(e) => setInputTitle(e.target.value)}
						className="h-11 w-full rounded-[8px] px-3.5 border border-neutral-200 outline-offset-0 outline-neutral-950 placeholder:text-neutral-400"
						placeholder={tab === "tenants" ? "Enter name" : "Enter title"}
					/>
				</label>
				{tab === "collections" && dialogMode === "create" ? (
					<label className="block">
						<div className="mb-1 text-sm font-medium text-neutral-700">Tenant ID</div>
						<input
							type="number"
							value={inputTenantId}
				...
				<div className="flex justify-end gap-2 pt-2">
					<Button title="Cancel" variant="secondary-gray" onClick={() => setIsDialogOpen(false)} />
					<Button title={dialogMode === "create" ? "Create" : "Save"} onClick={submitDialog} />
				</div>
			</div>
		</Dialog>
		</>
